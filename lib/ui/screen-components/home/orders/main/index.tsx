...
export default function HomeOrdersMain(props: IOrderTabsComponentProps) {
    ...
    const onInitOrders = () => {
        ...
        switch (route.key) {
            ...
            case "delivered":
                _orders = assignedOrders.filter(
                    (o: IOrder) =>
                        ['PICKED', 'ACCEPTED', 'DELIVERED'].includes(
                            o.orderStatus
                        ) &&
                        o.rider &&
    ...
    const marginBottom =
        Platform.OS === 'ios' ? height * 0.4 : height * 0.35;
    ...
    return (
        <View className='flex-1 bg-white'>
            ...
            {errorAssigned ? (
                <View className='flex-1 justify-center items-center'> <Text className='text-2xl'>Something went wrong</Text></View>
            ) : <FlatList
